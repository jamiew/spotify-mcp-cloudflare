...
**Status (2026-09-17):** reviewed the Python sibling's 0.5.0 (five contributor fixes, all found on a restricted app) for anything that applies here. One did: `get_playlist` dropped null rows, so positions drifted after a removed or local track. Fixed with placeholders. The rest were already covered by design choices made earlier: search pages at 10 unconditionally, track details did not batch, and `control_playback` returns text rather than a racy state read. Worth knowing from their live testing: `/artists/{id}/top-tracks` and `/tracks?ids=` 403 on restricted apps but still serve legacy ones, so both remain possible additions while this app is on the legacy regime.

**Status (2026-07-30, verified):** all 33 tools live against the deployed Worker; version is now `0.4.0` in both `package.json` and the MCP server info, with `check:meta` enforcing that they match. The nine new tools were each driven with real calls — cover upload accepted (202), follow and album round-trips verified by watching the totals move (323→324→323, 625→626→625) and reverted, and both cover rejection paths confirmed. The account was left exactly as found.

Verification lesson worth keeping: the first two "reconnect and retest" attempts didn't re-run Spotify's consent at all — the MCP client reused its stored token for our Worker, so `props` still carried the old grant. Only a full clear-auth produced "Authentication successful", and only then did the new scopes arrive. When a scope-related 403 survives a reconnect, confirm the browser consent screen actually appeared before suspecting the code.
...
- [x] Live-verify the nine tools added 2026-07-30 — all nine confirmed against the deployed Worker, including a cover upload accepted by Spotify (202), follow and save round-trips checked by total count and reverted, and both `set_playlist_cover` rejection paths (non-JPEG, unfetchable URL)
